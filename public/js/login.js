...
const MODO_EDICAO_LIVRE_LOGIN = false;
const API_URL = window.location.protocol === "file:"
  ? "http://localhost:8787"
  : window.location.origin;
const PAGINA_APOS_LOGIN = "/pages/home";
...
function mostrarMensagem(texto, tipo = "") {
  ...
  authMessage.classList.remove("erro", "sucesso", "aviso", "active", "error", "success");

  if (texto) {
    authMessage.classList.add("active");
  }

  if (tipo) {
    authMessage.classList.add(tipo);
    if (tipo === "erro") authMessage.classList.add("error");
    if (tipo === "sucesso") authMessage.classList.add("success");
  }
}
...
function salvarSessaoEdicaoLivre(cidadeSelecionada) {
  const cidadeFinal = normalizarCidadeLogin(cidadeSelecionada);
  let anterior = {};

  try {
    anterior = JSON.parse(localStorage.getItem("amor_usuario") || "null") || {};
  } catch {
    anterior = {};
  }
  ...
  const usuarioSessao = {
    ...
    id: "editor-livre",
    uid: "editor-livre",
    nome: anterior.nome || "Editor LAG",
    nomeCompleto: anterior.nomeCompleto || anterior.nome || "Editor LAG",
    nome_completo: anterior.nome_completo || anterior.nome || "Editor LAG",
    email: "editor@lag.local",
    nivelAcesso: "admin",
    nivel_acesso: "admin",
    cidade: cidadeFinal,
    cargo: anterior.cargo || "Administrador",
    setor: anterior.setor || "Edição",
    ativo: true,
    authProvider: "edicao-livre"
  };

  localStorage.setItem("amor_email", usuarioSessao.email);
  localStorage.setItem("usuarioEmail", usuarioSessao.email);
  localStorage.setItem("amor_usuario", JSON.stringify(usuarioSessao));
  localStorage.setItem("amorSaudeUsuario", JSON.stringify(usuarioSessao));
  localStorage.setItem("usuarioLogado", JSON.stringify(usuarioSessao));
  localStorage.setItem("amor_nivel_acesso", "admin");
  localStorage.setItem("nivelAcesso", "admin");
  localStorage.setItem("amor_cidade", cidadeFinal);
  localStorage.setItem("cidadeSelecionada", cidadeFinal);
  localStorage.setItem("amorSaudeCidadeSelecionada", cidadeFinal);
  localStorage.setItem("amorSaudeAuthProvider", "edicao-livre");
  localStorage.setItem("amorSaudeLogado", "true");
  localStorage.setItem("isLoggedIn", "true");
...
function configurarMostrarSenha() {
  ...
  togglePassword.addEventListener("click", () => {
    ...
    togglePassword.innerHTML = `<i class="fa-regular ${mostrar ? "fa-eye-slash" : "fa-eye"}"></i>`;
...
function configurarLogin() {
  ...
  loginForm.addEventListener("submit", async (event) => {
    ...
    if (MODO_EDICAO_LIVRE_LOGIN) {
      salvarSessaoEdicaoLivre(getCidadeLogin());
      mostrarMensagem("Modo de edição livre ativado. Abrindo painel...", "sucesso");
      setTimeout(() => window.location.replace(PAGINA_APOS_LOGIN), 250);
      return;
    }
