document.addEventListener("DOMContentLoaded", () => {
  ...
  const drawerCollapse = document.getElementById("homeDrawerCollapse");
  ...
  const SIDEBAR_STORAGE_KEY = "lagHomeSidebarCollapsedV2";
  ...
  function syncDesktopSidebar() {
    ...
    document.body.classList.toggle("lag-sidebar-desktop", desktop);

    if (!sideDrawer || !menuToggle) return;

    if (desktop) {
      const collapsed = localStorage.getItem(SIDEBAR_STORAGE_KEY) === "1";
      document.body.classList.toggle("lag-sidebar-collapsed", collapsed);
      ...
      menuToggle.setAttribute("aria-label", collapsed ? "Expandir menu lateral" : "Recolher menu lateral");
      drawerCollapse?.setAttribute("aria-expanded", String(!collapsed));
      drawerCollapse?.setAttribute("aria-label", collapsed ? "Expandir menu lateral" : "Recolher menu lateral");
      document.body.classList.remove("drawer-open", "menu-open");
      return;
    }

    document.body.classList.remove("lag-sidebar-collapsed");
    sideDrawer.classList.remove("active");
  ...
  function toggleMenu(event) {
    ...
    if (isDesktopSidebar()) {
      setDesktopSidebarCollapsed(!document.body.classList.contains("lag-sidebar-collapsed"));
      closeNotifications();
      closeProfile();
      return;
    }
  ...
  drawerCollapse?.addEventListener("click", toggleMenu);
  ...
  sideDrawer?.querySelectorAll("a, button").forEach((item) => {
    item.addEventListener("click", () => {
      if (item.classList.contains("home-drawer-close") || item.classList.contains("home-drawer-collapse")) return;
