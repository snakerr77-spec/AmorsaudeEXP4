...
async function loadCandidates(showOk = false) {
  try {
    const resposta = await window.AmorSaudeAPI.listar(CANDIDATES_COLLECTION);
    candidatesCache = (resposta.dados || resposta.items || [])
    ...
    if (candidateList) {
      candidateList.innerHTML = `
        <div class="empty-candidates">
          ...
          <p>Confira se o binding DB foi adicionado no Cloudflare.</p>
        </div>
      `;
    }
    setStatus(error.message || "Erro ao carregar médicos cadastrados do D1.", "erro");
  }
}

async function updateCandidateStatus(candidateId, status) {
  await window.AmorSaudeAPI.atualizar(CANDIDATES_COLLECTION, candidateId, {
    status,
...
async function approveCandidate(candidateId) {
  ...
  await window.AmorSaudeAPI.criar(DOCTORS_COLLECTION, {
    id: candidateId,
    nome: candidate.name || "",
    ...
    aprovadoPor: currentUser?.uid || currentUser?.id || "",
    aprovadoEm: new Date().toISOString(),
    atualizadoEm: new Date().toISOString()
  });

  await updateCandidateStatus(candidateId, "aprovado");
  setStatus("Médico aprovado e salvo no D1.", "sucesso");
}
...
function setupEvents() {
  ...
  document.addEventListener("click", async (event) => {
    ...
    try {
      ...
      setStatus(error.message || "Não foi possível concluir a ação no D1.", "erro");
    }
  });
}
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", () => setTimeout(() => init(window.usuarioLogado), 450));
} else {
  setTimeout(() => init(window.usuarioLogado), 450);
}
