...
function updateStats() {
  const activeFolders = new Set(state.docs.map((doc) => doc.folder)).size;
  const terms = state.docs.filter((doc) => normalize(doc.type).includes("termo") || normalize(doc.folder).includes("termos")).length;
  const values = state.docs.filter((doc) => normalize(doc.type).includes("valor") || normalize(doc.folder).includes("valores")).length;

  animateNumber(ui.statDocs, state.docs.length);
  animateNumber(ui.statFolders, activeFolders);
  animateNumber(ui.statTerms, terms);
  animateNumber(ui.statValues, values);

  if (ui.allDocumentsCount) ui.allDocumentsCount.textContent = `${state.docs.length} ${state.docs.length === 1 ? "arquivo" : "arquivos"}`;
  if (ui.folderCountBadge) ui.folderCountBadge.textContent = FOLDERS.length;

  const latest = [...state.docs].sort((a, b) => new Date(b.updatedAt || 0) - new Date(a.updatedAt || 0))[0];
...
function renderFolders() {
  ...
  FOLDERS.forEach((folder) => {
    const count = state.docs.filter((doc) => doc.folder === folder).length;
      ...
      <div class="document-top">
        <div class="doc-icon-title">
          ...
          <div>
            ...
            <small>${escapeHTML(doc.folder)}<br>${escapeHTML(relativeDate(doc.updatedAt))}</small>
          </div>
        </div>
...
function syncCity(city) {
  const finalCity = city || getCurrentCity();
  if (ui.controlCidadeSelect && [...ui.controlCidadeSelect.options].some((option) => option.value === finalCity)) ui.controlCidadeSelect.value = finalCity;
