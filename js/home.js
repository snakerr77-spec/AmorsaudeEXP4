...
// Busca e filtros da Home estática premium.
(() => {
  const input = document.getElementById("homeModuleSearch");
  const cards = [...document.querySelectorAll("[data-module-card]")];
  const filters = [...document.querySelectorAll("[data-home-filter]")];
  const empty = document.getElementById("homeModuleEmpty");
  ...
  function applyFilters() {
    ...
    cards.forEach((card) => {
      ...
      const searchable = normalize(`${card.dataset.moduleSearch || ""} ${card.textContent || ""}`);
      const matchesCategory = activeFilter === "todos" || category === activeFilter;
      const matchesSearch = !term || searchable.includes(term);
      const show = matchesCategory && matchesSearch && !card.hidden;
  ...
  filters.forEach((button) => {
    button.addEventListener("click", () => {
      activeFilter = button.dataset.homeFilter || "todos";
      filters.forEach((item) => item.classList.toggle("active", item === button));
      applyFilters();
    });
  });

  window.addEventListener("usuario-carregado", applyFilters);
  applyFilters();
})();
