...
// Login obrigatório: rotas internas usam sessão Cloudflare D1 e respeitam o perfil do usuário.
...
async function handleAssets(request, env) {
  ...
  // Mantém a tela de login original e permite o endereço sem .html.
  if (pathname === "/pages/login") {
    const loginUrl = new URL(request.url);
    loginUrl.pathname = "/pages/login.html";
    return env.ASSETS.fetch(new Request(loginUrl.toString(), request));
  }

  // URLs antigas continuam funcionando, mas apontam para as novas telas React.
  if (pathname === "/pages/home.html") {
    return Response.redirect(`${url.origin}/pages/home`, 302);
  }
  if (pathname === "/modules/prontuario-medico/prontuario-medico.html") {
    return Response.redirect(`${url.origin}/modules/prontuario-medico/prontuario-medico`, 302);
  }
  if (pathname === "/") {
    return Response.redirect(`${url.origin}/pages/login`, 302);
  }

  let response = await env.ASSETS.fetch(request);

  // As rotas React são entregues pelo index do Vite.
  if (response.status === 404 && wantsHtml(pathname)) {
