...
function isFreeEditMode(env) {
  return String(env?.FREE_EDIT_MODE || "false").trim().toLowerCase() === "true";
}
...
async function ensureDatabase(db, env = {}) {
  await db.prepare(`
  ...
  if (!Number(count?.total || 0)) {
    const adminEmail = normalizeEmail(env.INITIAL_ADMIN_EMAIL || "admin@lag.com");
    const adminPassword = String(env.INITIAL_ADMIN_PASSWORD || "123456");
    const senhaHash = await sha256(adminPassword);
    ...
    `).bind(uuid(), "Administrador LAG", "Administrador LAG", adminEmail, senhaHash, "admin", "Cerquilho", "", "Administrador", "Gestão", now, now).run();
  }

  if (isFreeEditMode(env)) {
...
function hasRole(user, roles = []) {
  return Boolean(user && roles.includes(normalizeRole(user.nivelAcesso || user.nivel_acesso)));
}
...
function canDeleteGeneric(user) {
  return hasRole(user, ["admin", "financeiro", "gerencia"]);
}

async function ensureDatabaseOnce(env) {
  if (!env.DB) throw new Error("Binding D1 DB ausente.");
  if (!databaseReadyPromise) {
    databaseReadyPromise = ensureDatabase(env.DB, env).catch((error) => {
...
async function route(request, env) {
  ...
  if (pathname === "/api" || pathname === "/api/health") {
    return json({
      ...
      modo_edicao_livre: isFreeEditMode(env),
  ...
  if (pathname === "/api/auth/login" && request.method === "POST") return login(request, env.DB, env);
  if (pathname === "/api/auth/logout" && request.method === "POST") return logout(request, env.DB);

  const freeEditMode = isFreeEditMode(env);
  const user = freeEditMode
  ...
  if (!collection || !/^[a-zA-Z0-9_-]{1,80}$/.test(collection) || ["auth", "me", "health"].includes(collection)) {
    return json({ ok: false, erro: "Rota não encontrada." }, 404);
  }

  const isPublicCandidateCreate = collection === "candidatosMedicos" && request.method === "POST" && !id;
  if (!freeEditMode && !user && !isPublicCandidateCreate) {
    return json({ ok: false, erro: "Faça login para acessar esta rota." }, 401);
  }

  if (collection === "usuarios") {
    if (!canManageUsers(user)) return json({ ok: false, erro: "Apenas administradores podem gerenciar usuários." }, 403);
    ...
    if (request.method === "DELETE" && id) {
      if (String(user?.id || "") === String(id)) return json({ ok: false, erro: "Você não pode excluir o próprio usuário conectado." }, 400);
      return deleteUser(env.DB, id);
    }
  }

  if (request.method === "GET") return listGeneric(env.DB, collection, url.searchParams.get("busca") || "");
  if (request.method === "POST" && (user || isPublicCandidateCreate)) return createGeneric(request, env.DB, collection, user);
  if (request.method === "PUT" && id) return updateGeneric(request, env.DB, collection, id);
  if (request.method === "DELETE" && id) {
    if (!canDeleteGeneric(user)) return json({ ok: false, erro: "Seu perfil não possui permissão para excluir registros." }, 403);
    return deleteGeneric(env.DB, collection, id);
  }
...
async function handleAssets(request, env) {
  ...
  // Permite URLs limpas como /pages/login e /modules/controladoria/controladoria.
  if (response.status === 404 && wantsHtml(url.pathname) && url.pathname !== "/") {
