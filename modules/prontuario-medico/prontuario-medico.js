...
async function refreshRecords(showOk = false) {
  ...
  try {
    ...
    setStatus("Erro ao carregar prontuários. Verifique se o usuário está logado e se as regras do Firestore liberam a coleção prontuariosMedicos.", "erro");
  } finally {
...
function setupFormControls() {
  ...
  form?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      setStatus("Erro ao salvar prontuário. Confira login, Firebase e regras da coleção prontuariosMedicos.", "erro");
    }
  });
}
