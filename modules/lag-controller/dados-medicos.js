...
const CARGOS_QUE_EDITAM_DADOS_MEDICOS = ["admin", "financeiro", "gerencia"];
...
async function carregarPermissaoDadosMedicosFirebase() {
  ...
  const usuarioCloud = window.usuarioLogado || (() => {
    for (const chave of ["amor_usuario", "amorSaudeUsuario", "usuarioLogado"]) {
      try {
  ...
  aplicarDadosUsuarioSistema({
    ativo: usuarioCloud?.ativo !== false,
    nivelAcesso: usuarioCloud?.nivelAcesso || usuarioCloud?.nivel_acesso || "colaborador"
  });
}
