...
// =========================================================
// PERMISSÃO DOS DADOS MÉDICOS
// Apenas admin e financeiro podem cadastrar, limpar ou remover.
// Colaborador, recepção e médico ficam somente visualização.
// =========================================================
const FIREBASE_CONFIG_DADOS_MEDICOS = {
...
const CARGOS_QUE_EDITAM_DADOS_MEDICOS = ["admin", "financeiro"];

let permissaoDadosMedicos = {
  carregando: true,
  ativo: false,
...
function normalizarNivelAcessoDadosMedicos(nivel) {
  ...
  if (valor === "administrador" || valor === "adm" || valor === "administrator") {
    return "admin";
  }

  if (valor === "financeira" || valor === "financial") {
    return "financeiro";
  }

  if (valor === "recepcao" || valor === "recepção") {
    return "recepcao";
  }

  return valor || "colaborador";
}

function usuarioPodeEditarDadosMedicos() {
  return permissaoDadosMedicos.podeEditar === true;
}

function bloquearAcaoDadosMedicos(mensagem = "Você não tem permissão para alterar dados médicos. Apenas admin e financeiro podem editar.") {
  if (usuarioPodeEditarDadosMedicos()) {
    return false;
  }
...
function aplicarPermissaoDadosMedicos() {
  const podeEditar = usuarioPodeEditarDadosMedicos();

  document.body.classList.toggle("dados-medicos-can-edit", podeEditar);
  document.body.classList.toggle("dados-medicos-readonly", !podeEditar);

  const aviso = document.getElementById("medicalReadonlyNotice");
  if (aviso) {
    aviso.hidden = podeEditar;
  }

  document.querySelectorAll(".admin-financeiro-only").forEach((elemento) => {
    elemento.setAttribute("aria-hidden", podeEditar ? "false" : "true");

    if ("disabled" in elemento) {
      elemento.disabled = !podeEditar;
    }
  });

  document.querySelectorAll(".doctor-form input, .doctor-form select, .doctor-form textarea, .doctor-form button").forEach((campo) => {
...
function aplicarDadosUsuarioSistema(dadosUsuario = {}) {
  const nivelNormalizado = normalizarNivelAcessoDadosMedicos(
    dadosUsuario.nivelAcesso ||
    dadosUsuario.role ||
    dadosUsuario.cargo ||
    dadosUsuario.tipoUsuario
  );

  const ativo = dadosUsuario.ativo === true;
  const podeEditar = ativo && CARGOS_QUE_EDITAM_DADOS_MEDICOS.includes(nivelNormalizado);

  permissaoDadosMedicos = {
    carregando: false,
    ativo,
    nivelAcesso: nivelNormalizado,
    podeEditar
  };
  ...
  console.log("Permissão dados médicos:", permissaoDadosMedicos);
}

async function carregarPermissaoDadosMedicosFirebase() {
  aplicarPermissaoDadosMedicos();
  ...
  try {
    ...
    const app = appModule.getApps().length
      ? appModule.getApp()
      : appModule.initializeApp(FIREBASE_CONFIG_DADOS_MEDICOS);

    const auth = authModule.getAuth(app);
    const db = firestoreModule.getFirestore(app);

    authModule.onAuthStateChanged(auth, async (usuario) => {
      if (!usuario) {
        aplicarDadosUsuarioSistema({
          ativo: false,
          nivelAcesso: "colaborador"
        });
        return;
      ...
      const usuarioRef = firestoreModule.doc(db, "usuarios", usuario.uid);
      const usuarioSnap = await firestoreModule.getDoc(usuarioRef);
      ...
      aplicarDadosUsuarioSistema(usuarioSnap.data());
    });
  } catch (erro) {
    console.warn("Não foi possível carregar permissão pelo Firebase. Mantendo somente visualização.", erro);
...
function normalizeDoctorsPeriods() {
  ...
  if (changed && usuarioPodeEditarDadosMedicos()) {
    saveDoctors();
  }
}
...
window.addEventListener("load", () => {
  ...
  aplicarPermissaoDadosMedicos();
  carregarPermissaoDadosMedicosFirebase();
  renderAll();
...
function loadDoctors() {
  ...
  // Colaborador não deve gerar ou alterar dados salvos localmente.
  // Se não houver nada salvo, mostramos apenas os dados iniciais em memória.
  if (!savedDoctors) {
...
function saveDoctors() {
  if (!usuarioPodeEditarDadosMedicos()) {
    console.warn("Bloqueado: usuário sem permissão para salvar dados médicos.");
    return false;
  }
...
function renderSavedDoctors() {
  ...
  if (!doctors.length) {
    list.innerHTML = `
      <div class="empty-doctors">
        ${usuarioPodeEditarDadosMedicos()
          ? "Nenhum médico cadastrado ainda. Adicione o primeiro médico."
          : "Nenhum médico cadastrado para visualização."}
      </div>
    `;
    return;
  }
    ...
    card.innerHTML = `
      ...
      ${usuarioPodeEditarDadosMedicos() ? `
        <div class="doctor-card-actions">
          <button class="delete-doctor-btn admin-financeiro-only" type="button" data-id="${doctor.id}">
  ...
  document.querySelectorAll(".delete-doctor-btn").forEach((button) => {
    button.addEventListener("click", () => {
      if (bloquearAcaoDadosMedicos("Você não tem permissão para remover médicos. Apenas admin e financeiro podem remover.")) {
        return;
      }

      const id = button.dataset.id;
      const confirmDelete = confirm("Tem certeza que deseja remover este médico?");
...
if (doctorForm) {
  doctorForm.addEventListener("submit", (event) => {
    ...
    if (bloquearAcaoDadosMedicos("Você não tem permissão para cadastrar médicos. Apenas admin e financeiro podem cadastrar.")) {
      return;
    }
...
if (clearDoctorsBtn) {
  clearDoctorsBtn.addEventListener("click", () => {
    if (bloquearAcaoDadosMedicos("Você não tem permissão para limpar dados médicos. Apenas admin e financeiro podem limpar.")) {
      return;
    }
...
function openPanel() {
  ...
  if (bloquearAcaoDadosMedicos("Seu acesso é somente visualização. Apenas admin e financeiro podem abrir o cadastro de médicos.")) {
    return;
  }
...
/* ==================================================
   MENU RESPONSIVO DO TOPO
   Controla o botão hambúrguer no celular/tablet.
================================================== */
