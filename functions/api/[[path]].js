...
async function route(request, env) {
  ...
  if (pathname === "/api" || pathname === "https://amorsaudeexp4.snakerr77.workers.dev/api/health") return json({ ok: true, api: "LAG Controller Cloudflare D1", db: true, horario: new Date().toISOString() });
  ...
  if (collection !== "candidatosMedicos" && !user) return json({ ok: false, erro: "Faça login para acessar esta rota." }, 401);
