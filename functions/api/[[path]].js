...
// Modo solicitado para esta cópia: todas as rotas podem ser usadas sem login.
// O usuário técnico abaixo recebe perfil de administrador para criar, editar e
// excluir dados em qualquer módulo.
const FREE_EDIT_MODE = true;
...
async function route(request, env) {
  ...
  if (pathname === "/api" || pathname === "/api/health") return json({ ok: true, api: "LAG Controller Cloudflare D1", db: true, horario: new Date().toISOString() });
...
export async function onRequest(context) {
  try { return await route(context.request, context.env); }
  catch (error) {
